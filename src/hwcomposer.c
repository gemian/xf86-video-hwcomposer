...
void hwc_start_fake_surfaceflinger(ScrnInfoPtr pScrn) {
    ...
    if (startMiniSurfaceFlinger) {
        ...
        xf86DrvMsg(pScrn->scrnIndex, X_WARNING,
                   "libminisf is incompatible or missing. Can not possibly start the fake SurfaceFlinger service.\n");
    }
}
...
Bool hwc_hwcomposer_init(ScrnInfoPtr pScrn)
{
    ...
    hwc->primary_display.hwcContents = (hwc_display_contents_1_t **) malloc(
        HWC_NUM_DISPLAY_TYPES * sizeof(hwc_display_contents_1_t * ));
...
void hwc_toggle_screen_brightness(ScrnInfoPtr pScrn)
{
    ...
    xf86DrvMsg(pScrn->scrnIndex, X_INFO,
               "hwc_toggle_screen_brightness brightness: %d, pDpms: %d, eDpms: %d, hwcBrightness: %d\n", brightness,
               hwc->primary_display.dpmsMode, hwc->external_display.dpmsMode, hwc->screenBrightness);
